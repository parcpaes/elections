...
router.get('/:id', async (req, res) => {
    ...
    if (!acta) return res.status(404).send('The Acta with the given ID was not found.');    
    res.send(acta);
});
...
router.post('/',uploadFile.single('file'),async (req, res) => {
  ...
  if(imagerror.error) return res.status(400).send(imagerror.error.details[0].message);
