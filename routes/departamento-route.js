...
router.get('/:id', async (req, res) => {
    ...
    if (!departamento) return res.status(404).send('The departamento with the given ID was not found.');
    res.send(departamento);
});
...
router.post('/', async (req, res) => {
  ...
  if(isDepartamento) return res.status(400).send('Departamento already register');
...
router.put('/:id', async (req, res) => {
    ...
    const departamento = await Departamento.findByIdAndUpdate(req.params.id,{
      name:req.body.name
    },{new:true});
